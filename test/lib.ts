import { CallProxy, DeBridgeGate, SignatureVerifier } from "../src/evm/typechain";
...
export async function deployGate(opts: DeBridgeGateOpts = {}): Promise<DeBridgeGate> {
  opts.validators = opts.validators || []
    ...
    if (opts.validators!.length === 0) {
      verifier = await deployMockContract(
      ...
      verifier = await hre.upgrades.deployProxy(
        Verifier, [
