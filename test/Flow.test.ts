...
import { EVMContext } from "../src/evm";
import { SignersSignatureStorage } from "../src/evm";
...
async function deployContracts(signers: ethers.Signer[]): Promise<TestSuiteState> {
...
describe("General flow", function () {
  ...
  it("Must capture one submission", async function () {
    const submissions = await evm.EVMSubmission.from(this.tx.transactionHash, this.evmContext);
  ...
  it("Must claim", async function () {
    ...
    await this.contracts.gate.claim(
      ...args
    );

    expect(await this.contracts.counter.counter())
...
describe("General flow: multiple submissions per one txn", function () {
  ...
  it("Must capture multiple submissions", async function () {
    this.submissions = await evm.EVMSubmission.from(this.tx.transactionHash, this.evmContext);

    expect(this.submissions.length)
      .to.be.eq(3);
  });

  for (let i = 0; i < 3; i++) {
    it(`Must claim #${i + 1}`, async function () {
      const preCounterValue = (await this.contracts.counter.counter()).toNumber();
      ...
      expect(await this.contracts.counter.counter())
        .to.be.eq(preCounterValue + INCREMENT_BY * (i + 1))
    });
  }
